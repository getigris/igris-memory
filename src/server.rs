...
fn lock_db(db: &Arc<Mutex<Database>>) -> Result<std::sync::MutexGuard<'_, Database>, String> {
    db.lock().map_err(|e| format!("Lock error: {e}"))
}
...
impl IgrisServer {
    ...
    fn igris_get(&self, Parameters(args): Parameters<GetArgs>) -> String {
        ...
        match db.get_observation(args.id) {
            Ok(obs) => to_json(&obs),
    ...
    fn igris_delete(&self, Parameters(args): Parameters<DeleteArgs>) -> String {
        ...
        match db.delete_observation(args.id) {
            Ok(true) => r#"{"deleted": true}"#.to_string(),
            Ok(false) => "Not found or already deleted".to_string(),
    ...
    fn igris_context(&self, Parameters(args): Parameters<ContextArgs>) -> String {
        ...
        match db.recent_context(args.project.as_deref(), args.limit) {
            Ok(observations) => to_json(&observations),
            Err(e) => format!("Context error: {e}"),
        }
    }
    ...
    fn igris_session_start(&self, Parameters(args): Parameters<SessionStartArgs>) -> String {
        ...
        match db.start_session(&args.id, &args.project, args.directory.as_deref()) {
            Ok(session) => to_json(&session),
            Err(e) => format!("Session start error: {e}"),
        }
    }
    ...
    fn igris_session_end(&self, Parameters(args): Parameters<SessionEndArgs>) -> String {
        ...
        match db.end_session(&args.id, args.summary.as_deref()) {
            Ok(session) => to_json(&session),
            Err(e) => format!("Session end error: {e}"),
        }
    }
    ...
    fn igris_session_summary(&self, Parameters(args): Parameters<SessionSummaryArgs>) -> String {
        ...
        match db.save_session_summary(&args.content, &args.project) {
            Ok(session) => to_json(&session),
            Err(e) => format!("Session summary error: {e}"),
        }
    }
}
