...
fn cli_parses_data_dir() {
    let cli = Cli::try_parse_from(["igmem", "--data-dir", "/tmp/test"]).unwrap();
    assert_eq!(cli.data_dir, Some(PathBuf::from("/tmp/test")));
}

#[test]
fn cli_no_args_ok() {
    let cli = Cli::try_parse_from(["igmem"]).unwrap();
    assert_eq!(cli.data_dir, None);
}

#[test]
fn cli_resolve_flag_takes_priority() {
    let cli = Cli::try_parse_from(["igmem", "--data-dir", "/custom"]).unwrap();
...
#[test]
fn cli_version_flag() {
    let result = Cli::try_parse_from(["igmem", "--version"]);
...
#[test]
fn cli_help_flag() {
    let result = Cli::try_parse_from(["igmem", "--help"]);
    assert!(result.is_err());
    let err = result.unwrap_err();
...
fn cli_unknown_flag_fails() {
    let result = Cli::try_parse_from(["igmem", "--bogus"]);
    assert!(result.is_err());
}

#[test]
fn cli_serve_default_port() {
    let cli = Cli::try_parse_from(["igmem", "serve"]).unwrap();
    match cli.command {
...
fn cli_serve_custom_port() {
    let cli = Cli::try_parse_from(["igmem", "serve", "--port", "8080"]).unwrap();
    match cli.command {
...
fn resolve_global_db_path() {
    let cli = Cli::try_parse_from(["igmem", "--data-dir", "/tmp/igris"]).unwrap();
...
fn cli_parses_db_key() {
    let cli = Cli::try_parse_from(["igmem", "--db-key", "secret123"]).unwrap();
